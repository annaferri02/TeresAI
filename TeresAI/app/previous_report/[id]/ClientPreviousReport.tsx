...
const ALLOWED_SECTIONS = [
  "Eating Behaviour", "Sleep", "Medicine Administration",
  "Energy", "Pain Complaints", "Stool", "Blood Pressure"
];
  ...
  <div className="p-4 rounded-md bg-white text-custom-dark-purple border border-custom-lilac w-full max-w-4xl">
    <h3 className="text-lg font-semibold flex justify-between items-center">
      ...
      {!isEditing && ALLOWED_SECTIONS.includes(title) && !isLocked && (
        <button onClick={onEdit} className="text-custom-dark-purple hover:text-custom-lilac">
        </button>
      )}
    </h3>
...
export default function ClientTranscriptPage({ id, patientName }: { id: string; patientName: string }) {
